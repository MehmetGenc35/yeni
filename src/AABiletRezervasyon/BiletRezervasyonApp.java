...
public class BiletRezervasyonApp {
    public static void main(String[] args) {
        ...
        start(bus,ticket,ticketService,yolcu,yolcuService);
    }
    public static void start(Bus bus, Ticket ticket,TicketService ticketService,Yolcu yolcu,YolcuService yolcuService){
        Scanner inp=new Scanner(System.in);
        int select;///ÇIKIŞ için 0 seçiniz.
        do {
            yolcuService.informationPassenger(bus,yolcu,ticket);
            //System.out.println(yolcu.getAge());
            ticketService.getTotalPrice(ticket,yolcu);

            //bileti yazdıralım
            ticketService.printTicket(bus,ticket,yolcu);
